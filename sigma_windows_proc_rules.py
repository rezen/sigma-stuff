...
def sigma_application_whitelisting_bypass_via_dxcap_exe(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'Image']
    level: medium
...
def sigma_application_whitelisting_bypass_via_dll_loaded_by_odbcconf_exe(record):
    ...
    fields: ['OriginalFilename', 'CommandLine', 'Image', 'ParentImage']
    level: medium
...
def sigma_suspicious_child_process_created_as_system(record):
    ...
    fields: ['IntegrityLevel', 'User', 'Image', 'ParentUser', 'CommandLine']
    level: high
...
def sigma_sideloading_link_exe(record):
    ...
    fields: ['CommandLine', 'ParentImage', 'Image']
    level: medium
...
def sigma_renamed_paexec(record):
    ...
    fields: ['OriginalFileName', 'Image', 'Description']
    level: high
...
def sigma_using_settingsynchost_exe_as_lolbin(record):
    ...
    fields: ['ParentCommandLine', 'Image']
    level: high
...
def sigma_suspicious_powershell_parent_process(record):
    ...
    fields: ['ParentImage', 'Product', 'Image', 'CommandLine', 'Description']
    level: high
...
def sigma_suspicious_userinit_child_process(record):
    ...
    fields: ['ImageFileName', 'CommandLine', 'ParentImage', 'Image']
    level: medium
...
def sigma_suspicious_csc_exe_source_file_folder(record):
    ...
    fields: ['ParentCommandLine', 'CommandLine', 'ParentImage', 'Image']
    level: medium
...
def sigma_malicious_windows_script_components_file_execution_by_taef_detection(record):
    ...
    fields: ['OriginalFileName', 'Image', 'ParentImage']
    level: low
...
def sigma_uac_bypass_using_msconfig_token_modification_process(record):
    ...
    fields: ['IntegrityLevel', 'CommandLine', 'ParentImage']
    level: high
...
def sigma_uac_bypass_abusing_winsat_path_parsing_process(record):
    ...
    fields: ['ParentCommandLine', 'IntegrityLevel', 'ParentImage']
    level: high
...
def sigma_rubeus_hack_tool(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'Image', 'Description']
    level: critical
...
def sigma_use_of_anydesk_remote_access_software_from_suspicious_folder(record):
    ...
    fields: ['Image', 'Company', 'Product', 'Description']
    level: high
...
def sigma_dism_remove_online_package(record):
    ...
    fields: ['ParentCommandLine', 'Image', 'CommandLine']
    level: medium
...
def sigma_suspicious_schtasks_from_env_var_folder(record):
    ...
    fields: ['ParentCommandLine', 'CommandLine', 'Image']
    level: high
...
def sigma_possible_privilege_escalation_via_service_permissions_weakness(record):
    ...
    fields: ['IntegrityLevel', 'CommandLine']
    level: high
...
def sigma_cmd_exe_commandline_path_traversal(record):
    ...
    fields: ['ParentCommandLine', 'CommandLine', 'ParentImage']
    level: high
...
def sigma_renamed_adfind_detection(record):
    ...
    fields: ['Imphash', 'Image', 'OriginalFileName', 'CommandLine', 'Hashes']
    level: high
...
def sigma_iox_tunneling_tool(record):
    ...
    fields: ['sha256', 'Image', 'sha1', 'CommandLine', 'md5', 'Hashes']
    level: high
...
def sigma_use_of_logmein_remote_access_software(record):
    ...
    fields: ['Company', 'Product', 'Description']
    level: medium
    description: An adversary may use legitimate desktop support and remote access software, such as Team Viewer, Go2Assist, LogMein, AmmyyAdmin, etc, to establish an interactive command and control channel to target systems within networks.
These services are commonly used as legitimate technical support software, and may be allowed by application control within a target environment.
Remote access tools like VNC, Ammyy, and Teamviewer are used frequently when compared with other legitimate software commonly used by adversaries. (Citation: Symantec Living off the Land)
...
def sigma_automated_collection_command_prompt(record):
    ...
    fields: ['OriginalFileName', 'CommandLine']
    level: medium
...
def sigma_uac_bypass_via_windows_firewall_snap_in_hijack(record):
    ...
    fields: ['ParentCommandLine', 'Image', 'ParentImage']
    level: medium
...
def sigma_always_install_elevated_windows_installer(record):
    ...
    fields: ['IntegrityLevel', 'ParentImage', 'User', 'Image', 'CommandLine', 'ParentCommandLine']
    level: medium
...
def sigma_use_of_pdq_deploy_remote_adminstartion_tool(record):
    ...
    fields: ['OriginalFileName', 'Company', 'Product', 'Description']
    level: medium
...
def sigma_exploiting_cve_2019_1388(record):
    ...
    fields: ['IntegrityLevel', 'ParentImage', 'User', 'Image', 'CommandLine']
    level: critical
...
def sigma_renamed_megasync(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'ParentImage', 'Image']
    level: high
...
def sigma_renamed_sysinternals_debug_view(record):
    ...
    fields: ['OriginalFileName', 'Image', 'Product']
    level: high
...
def sigma_suspicious_powershell_sub_processes(record):
    ...
    fields: ['ParentCommandLine', 'CommandLine', 'Image', 'ParentImage']
    level: high
...
def sigma_wmiprvse_spawning_process(record):
    ...
    fields: ['LogonId', 'Image', 'ParentImage', 'User']
    level: high
...
def sigma_use_short_name_path_in_image(record):
    ...
    fields: ['ParentImage', 'Product', 'Image', 'Company', 'Description']
    level: high
...
def sigma_process_hacker_system_informer_usage(record):
    ...
    fields: ['Product', 'sha256', 'Image', 'sha1', 'OriginalFileName', 'imphash', 'md5', 'Hashes', 'Description']
    level: high
...
def sigma_potential_suspicious_activity_using_secedit(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'Image', 'SubjectUserName']
    level: medium
...
def sigma_nimgrab_file_download(record):
    ...
    fields: ['sha256', 'Image', 'imphash', 'md5', 'Hashes']
    level: high
...
def sigma_chcp_codepage_locale_lookup(record):
    ...
    fields: ['ParentImage', 'GrandparentCommandLine', 'Image', 'CommandLine', 'ParentCommandLine']
    level: high
...
def sigma_ppid_spoofing_tool_usage(record):
    ...
    fields: ['Imphash', 'Image', 'OriginalFileName', 'CommandLine', 'Hashes', 'Description']
    level: high
...
def sigma_accesschk_usage_to_check_privileges(record):
    ...
    fields: ['Product', 'Image', 'OriginalFileName', 'CommandLine', 'Description']
    level: medium
...
def sigma_hiding_files_with_attrib_exe(record):
    ...
    fields: ['ParentImage', 'Image', 'OriginalFileName', 'CommandLine', 'ParentCommandLine']
    level: low
...
def sigma_uac_bypass_using_disk_cleanup(record):
    ...
    fields: ['IntegrityLevel', 'ParentCommandLine', 'CommandLine']
    level: high
...
def sigma_windows_credential_editor(record):
    ...
    fields: ['ParentImage', 'Imphash', 'Image', 'CommandLine', 'Hashes']
    level: critical
...
def sigma_possible_privilege_escalation_via_weak_service_permissions(record):
    ...
    fields: ['IntegrityLevel', 'CommandLine', 'Image']
    level: high
...
def sigma_formbook_process_creation(record):
    ...
    fields: ['ParentCommandLine', 'CommandLine']
    level: high
...
def sigma_run_whoami_as_privileged_user(record):
    ...
    fields: ['OriginalFileName', 'Image', 'User']
    level: high
...
def sigma_stop_windows_service(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'Image', 'User']
    level: low
...
def sigma_pchunter_usage(record):
    ...
    fields: ['sha256', 'Image', 'sha1', 'OriginalFileName', 'imphash', 'md5', 'Hashes', 'Description']
    level: high
...
def sigma_gmer_rootkit_detector_and_remover_execution(record):
    ...
    fields: ['SHA1', 'Image', 'SHA256', 'Hashes', 'MD5']
    level: high
...
def sigma_suspicious_compression_tool_parameters(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'ParentImage']
    level: high
...
def sigma_suspicious_file_characteristics_due_to_missing_fields(record):
    ...
    fields: ['Product', 'Image', 'Company', 'FileVersion', 'Description']
    level: medium
...
def sigma_conhost_parent_process_executions(record):
    ...
    fields: ['ParentImage', 'Image', 'Provider_Name', 'CommandLine', 'ParentCommandLine']
    level: medium
...
def sigma_shells_spawned_by_web_servers(record):
    ...
    fields: ['ParentCommandLine', 'Image', 'ParentImage', 'CommandLine']
    level: high
...
def sigma_uac_bypass_via_icmluautil(record):
    ...
    fields: ['OriginalFileName', 'ParentCommandLine', 'Image', 'ParentImage']
    level: high
...
def sigma_suspicious_csi_exe_usage(record):
    ...
    fields: ['OriginalFileName', 'Image', 'Company']
    level: medium
...
def sigma_bloodhound_and_sharphound_hack_tool(record):
    ...
    fields: ['Product', 'Image', 'CommandLine', 'Company', 'Description']
    level: high
...
def sigma_uac_bypass_using_ntfs_reparse_point_process(record):
    ...
    fields: ['IntegrityLevel', 'ParentCommandLine', 'CommandLine', 'Image']
    level: high
...
def sigma_uac_bypass_tool_uacme_akagi(record):
    ...
    fields: ['Product', 'Imphash', 'Image', 'OriginalFileName', 'Company', 'Hashes', 'Description']
    level: high
...
def sigma_windows_shell_spawning_suspicious_program(record):
    ...
    fields: ['ParentImage', 'Image', 'CommandLine', 'CurrentDirectory', 'ParentCommandLine']
    level: high
...
def sigma_read_and_execute_a_file_via_cmd_exe(record):
    ...
    fields: ['OriginalFileName', 'ParentCommandLine', 'Image', 'CommandLine']
    level: medium
...
def sigma_ie4uinit_lolbin_use_from_invalid_path(record):
    ...
    fields: ['OriginalFileName', 'Image', 'CurrentDirectory']
    level: medium
...
def sigma_abused_debug_privilege_by_arbitrary_parent_processes(record):
    ...
    fields: ['ParentImage', 'User', 'Image', 'OriginalFileName', 'CommandLine']
    level: high
...
def sigma_suspicious_spool_service_child_process(record):
    ...
    fields: ['CommandLine', 'IntegrityLevel', 'ParentImage', 'Image']
    level: high
...
def sigma_suspicious_razerinstaller_explorer_subprocess(record):
    ...
    fields: ['IntegrityLevel', 'ParentImage', 'Image']
    level: high
...
def sigma_execution_of_renamed_paexec(record):
    ...
    fields: ['Image', 'Imphash', 'Hashes', 'Product']
    level: medium
...
def sigma_suspicious_xor_encoded_powershell_command_line(record):
    ...
    fields: ['CommandLine', 'ParentImage', 'Product', 'Description']
    level: medium
...
def sigma_uac_bypass_using_windows_media_player_process(record):
    ...
    fields: ['IntegrityLevel', 'ParentCommandLine', 'Image']
    level: high
...
def sigma_execution_of_renamed_netsupport_rat(record):
    ...
    fields: ['Imphash', 'Product', 'Image', 'OriginalFileName', 'Hashes']
    level: high
...
def sigma_handlekatz_lsass_dumper_usage(record):
    ...
    fields: ['CommandLine', 'Imphash', 'Hashes', 'Image']
    level: high
...
def sigma_rclone_execution_via_command_line_or_powershell(record):
    ...
    fields: ['CommandLine', 'ParentImage', 'Image', 'Description']
    level: high
...
def sigma_use_of_ultraviewer_remote_access_software(record):
    ...
    fields: ['OriginalFileName', 'Company', 'Product']
    level: medium
    description: An adversary may use legitimate desktop support and remote access software, such as Team Viewer, Go2Assist, LogMein, AmmyyAdmin, etc, to establish an interactive command and control channel to target systems within networks.
These services are commonly used as legitimate technical support software, and may be allowed by application control within a target environment.
Remote access tools like VNC, Ammyy, and Teamviewer are used frequently when compared with other legitimate software commonly used by adversaries. (Citation: Symantec Living off the Land)
...
def sigma_recon_information_for_export_with_command_prompt(record):
    ...
    fields: ['OriginalFileName', 'ParentCommandLine', 'Image']
    level: medium
    description: Once established within a system or network, an adversary may use automated techniques for collecting internal data.
    logsource: product:windows - category:process_creation
    """
...
def sigma_sharpldapwhoami(record):
    ...
    fields: ['OriginalFileName', 'CommandLine', 'Image', 'Product']
    level: high
